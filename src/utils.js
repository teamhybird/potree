...
export class Utils {
  ...
  static moveTo(scene, endPosition, endTarget) {
...

    {
      ...
      let target = new THREE.Vector3().addVectors(camera.position, camera.getWorldDirection(new THREE.Vector3()).clone().multiplyScalar(camTargetDistance));
  ...
  static findClosestGpsTime(target, viewer) {
    ...
    for (const node of nodes) {
      const isOkay = node.geometryNode != null && node.geometryNode.geometry != null && node.sceneNode != null;
  ...
  static lineToLineIntersection(P0, P1, P2, P3) {
    ...
    const d = (m, n, o, p) => {
      let result = (P[m].x - P[n].x) * (P[o].x - P[p].x) + (P[m].y - P[n].y) * (P[o].y - P[p].y) + (P[m].z - P[n].z) * (P[o].z - P[p].z);
