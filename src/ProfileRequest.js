  ...
  *updateGenerator() {
    ...
    for (let i = 0; i < Math.min(maxNodesPerUpdate, this.priorityQueue.size()); i++) {
      ...
      if (node.loaded) {
        ...
        var doTraverse = node.getLevel() === 0 || (node.level % hierarchyStepSize === 0 && node.hasChildren);
  ...
  *getPointsInsideProfile(nodes, target) {
    ...
    for (let segment of target.segments) {
      for (let node of nodes) {
        ...
        let matrix = new THREE.Matrix4().multiplyMatrices(this.pointcloud.matrixWorld, nodeMatrix);
