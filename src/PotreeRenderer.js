...
export class Renderer {
  ...
  renderNodes(octree, nodes, visibilityTextureData, camera, target, shader, params) {
    ...
    for (let node of nodes) {
      ...
      if (geometry.attributes['gps-time']) {
        ...
        let normalizedClipRange = [(uFilterGPSTimeClipRange[0] - globalRange[0]) / globalRangeSize, (uFilterGPSTimeClipRange[1] - globalRange[0]) / globalRangeSize];
      ...
      let isExtraAttribute = attributeLocations[material.activeAttributeName] === undefined && Object.keys(geometry.attributes).includes(material.activeAttributeName);
  ...
  renderOctree(octree, nodes, camera, target, params = {}) {
    ...
    if (material.pointSizeType >= 0) {
      if (material.pointSizeType === PointSizeType.ADAPTIVE || material.activeAttributeName === 'level of detail') {
...

    {
      ...
      shader.setUniform3f('uIntensity_gbc', [material.intensityGamma, material.intensityBrightness, material.intensityContrast]);

      shader.setUniform3f('uRGB_gbc', [material.rgbGamma, material.rgbBrightness, material.rgbContrast]);
