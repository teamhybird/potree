...
export class InputHandler extends EventDispatcher {
  ...
  onMouseDown(e) {
    ...
    let target = this.hoveredElements.find((el) => el.object._listeners && el.object._listeners['drag'] && el.object._listeners['drag'].length > 0);
  ...
  onMouseUp(e) {
    ...
    if (this.hoveredElements.length > 0) {
      let hovered = this.hoveredElements.map((e) => e.object).find((e) => e._listeners && e._listeners['mouseup']);
      if (hovered) {
  ...
  onMouseMove(e) {
    ...
    if (this.drag) {
      ...
      if (hoveredElements.length > 0) {
        let object = hoveredElements.map((e) => e.object).find((e) => e._listeners && e._listeners['mousemove']);
  ...
  getMousePointCloudIntersection(mouse) {
    return Utils.getMousePointCloudIntersection(this.mouse, this.scene.getActiveCamera(), this.viewer, this.scene.pointclouds);
  }
