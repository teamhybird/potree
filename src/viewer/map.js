...
export class MapView {
  ...
  init() {
    ...
    this.map = new ol.Map({
      ...
      layers: [new ol.layer.Tile({ source: new ol.source.OSM() }), this.toolLayer, this.annotationsLayer, this.sourcesLayer, this.sourcesLabelLayer, this.images360Layer, extentsLayer, cameraLayer],
  ...
  getMapCenter() {
    ...
    let mapCenter = [(mapExtent.bottomLeft[0] + mapExtent.topRight[0]) / 2, (mapExtent.bottomLeft[1] + mapExtent.topRight[1]) / 2];
  ...
  async load(pointcloud) {
    ...
    this.gExtent.setCoordinates([mapExtent.bottomLeft, mapExtent.bottomRight, mapExtent.topRight, mapExtent.topLeft, mapExtent.bottomLeft]);
    ...
    fetch(url)
      .then(async (response) => {
        ...
        for (let i = 0; i < sources.length; i++) {
          ...
          let mapCenter = [(mapBounds.min[0] + mapBounds.max[0]) / 2, (mapBounds.min[1] + mapBounds.max[1]) / 2];
